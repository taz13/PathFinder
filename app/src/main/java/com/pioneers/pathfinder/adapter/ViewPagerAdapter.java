...
public class ViewPagerAdapter extends FragmentStatePagerAdapter {
    ...
    public Fragment getItem(int position) {
        ...
        {
            ShortestPathFragment shortesPathTab = new ShortestPathFragment();
            return shortesPathTab;
        }
        else if(position == 1) // if the position is 0 we are returning the First tab
        {
            CheapestPathFragment cheapestPathTab = new CheapestPathFragment();
            return cheapestPathTab;
        }
        else if(position == 2)            // As we are having 4 tabs if the position is now 0 it must be 2 so we are returning third tab
        {
            BusStopsFragment busStopTab = new BusStopsFragment();
            return busStopTab;
        }
